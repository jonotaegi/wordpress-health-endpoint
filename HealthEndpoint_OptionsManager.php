...
class HealthEndpoint_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'health-endpoint'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'health-endpoint'); ?></h2>
            <table cellspacing="1" cellpadding="2"><tbody>
            <tr><td><?php _e('System', 'health-endpoint'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'health-endpoint'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'health-endpoint');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'health-endpoint'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'health-endpoint');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'health-endpoint'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'health-endpoint') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'health-endpoint');
            case 'false':
                return __('false', 'health-endpoint');

            case 'Administrator':
                return __('Administrator', 'health-endpoint');
            case 'Editor':
                return __('Editor', 'health-endpoint');
            case 'Author':
                return __('Author', 'health-endpoint');
            case 'Contributor':
                return __('Contributor', 'health-endpoint');
            case 'Subscriber':
                return __('Subscriber', 'health-endpoint');
            case 'Anyone':
                return __('Anyone', 'health-endpoint');
        }
        return $optionValue;
    }
